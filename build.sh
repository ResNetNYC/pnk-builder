...
used_mktemp=false
: ${PNK_RPI_IMAGE_URL:="https://downloads.raspberrypi.org/raspbian_lite/images/raspbian_lite-2019-09-30/2019-09-26-raspbian-buster-lite.zip"}
: ${PNK_RPI_IMAGE_SHA256SUM:="a50237c2f718bd8d806b96df5b9d2174ce8b789eda1f03434ed2213bbca6c6ff"}
: ${PNK_TEMP_DIR:="$(
    used_mktemp=true
    mktemp -d
)"}
...
EOF
    ...
    apt-get -qq update && \
    apt-get install -y --no-install-recommends curl git libffi-dev python python-dev python-setuptools python-pip python-backports.ssl-match-hostname python-cffi python-nacl libssl-dev pwgen haveged" || {
